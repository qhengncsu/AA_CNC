...
lambdas = 5:5:40;
R2_val_convex_original = (1.25-mses_validation_convex_original_DY)/1.25;
R2_val_convex_aa2 = (1.25-mses_validation_convex_aa2_DY)/1.25;
R2_val_cnc_original = (1.25-mses_validation_cnc_original_DY)/1.25;
R2_val_cnc_aa2 = (1.25-mses_validation_cnc_aa2_DY)/1.25;
R2_test_convex_original = (1.25-mses_test_convex_original_DY)/1.25;
R2_test_convex_aa2 = (1.25-mses_test_convex_aa2_DY)/1.25;
R2_test_cnc_original = (1.25-mses_test_cnc_original_DY)/1.25;
R2_test_cnc_aa2 = (1.25-mses_test_cnc_aa2_DY)/1.25;
hours_cnc_original = sum(times_cnc_original_DY)/3600;
hours_cnc_aa2 = sum(times_cnc_aa2_DY)/3600;
hours_convex_original = sum(times_convex_original_DY)/3600;
hours_convex_aa2 = sum(times_convex_aa2_DY)/3600;

t=tiledlayout(2,2, 'Padding', 'none', 'TileSpacing', 'compact'); 
set(gcf,'Position',[10 10 800 800])
nexttile
plot(log(res_norm_hist_convex_original_DY), 'b-', 'LineWidth', 1.5, 'DisplayName', 'Original')
hold on
plot(log(res_norm_hist_convex_aa2_DY), 'r-', 'LineWidth', 1.5, 'DisplayName', 'AA')
xlabel('iteration');
ylabel('residual norm (log scale)');
title('$$\gamma=0, \lambda=40$$','Interpreter','latex')
l = legend('show','Location','northeast','fontsize',10)
nexttile
plot(log(res_norm_hist_cnc_original_DY), 'b-', 'LineWidth', 1.5, 'DisplayName', 'Original')
hold on
plot(log(res_norm_hist_cnc_aa2_DY), 'r-', 'LineWidth', 1.5, 'DisplayName', 'AA')
xlabel('iteration');
ylabel('residual norm (log scale)');
