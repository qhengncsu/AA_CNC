...
for i = 1:length(lambda1_range)
    for j = 1:length(lambda2_range)
        ...
        for k = 1:n_folds
            ...
            [xhat_DR, res_norm_hist_DR] = cvxmin(A_train, Y_train(:), mu, z0, app, 'lambda1', lambda1_range(i), 'lambda2', lambda2_range(j),...
                                                 'max_iter', 1e4,  'splitting', 'DR', 'acceleration', 'original');
...
csvwrite('cv_scores_DR.csv',cv_scores)
%Xhat = srls_GMC_sprr(Y_meta_train, X_meta_train, best_lambda1, best_lambda2, 'acceleration', 'original', 'gamma', 0.6);
[xhat_DR, res_norm_hist_DR] = cvxmin(A_meta_train, Y_meta_train(:), mu, z0, app, 'lambda1', best_lambda1, 'lambda2', best_lambda2,...
                                                 'max_iter', 1e4, 'splitting', 'DR', 'acceleration', 'original');
z = ( xhat_DR(1:(p*L)) + xhat_DR((p*L+1):(2*p*L)) + xhat_DR((2*p*L+1):end))/3;
Xhat = reshape(z, [p, L]);                                       
...
save DR_CV.mat best_lambda1 best_lambda2 max_r2 r2_test