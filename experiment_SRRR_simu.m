...
p = 200;
L = 20;
s = 5; % number of nonzero rows in X, <p

A = randn(n, p);
X = rand(p, L);
X(randsample(p, p-s), :) = 0;
...
[xhat_DR, res_norm_hist_DR] = cvxmin(A, b, mu, z0, app, 'lambda1', lambda1, 'lambda2', lambda2,...
                                 'max_iter', 1e4, 'acceleration', 'original', 'splitting', 'DR', 'printevery', 10);
t_DR = toc(t0);


t0 = tic;
[xhat_DR_aa, res_norm_hist_DR_aa] = cvxmin(A, b, mu, z0, app, 'lambda1', lambda1, 'lambda2', lambda2,...
                                     'max_iter', 1e4, 'splitting', 'DR', 'printevery', 10);
t_DR_aa = toc(t0);
