...
% vanilla
[x_fb, v_fb, res_norm_fb, ~, t1] = srls_GMC_acc(y, X, lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', 'original',"gamma",gamma, 'splitting', 'FB');


% AA
[x_fbaa, v_fbaa, res_norm_fbaa, ~, t2] = srls_GMC_acc(y, X,  lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', "aa2","gamma",gamma, 'splitting', 'FB');

%%
[x_fbf, v_fbf, res_norm_fbf, ~, t3] = srls_GMC_acc(y, X, lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', 'original',"gamma",gamma, 'splitting', 'FBF');


[x_fbfaa, v3_fbfaa, res_norm_fbfaa, ~, t4] = srls_GMC_acc(y, X,  lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', "aa2","gamma",gamma, 'splitting', 'FBF');

%%
%% algorithm comparison on GMC: DR v.s. DR+AA
% vanilla
[x_dr, v_dr, res_norm_dr, ~, t5] = srls_GMC_acc(y, X, lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', 'original',"gamma",gamma, 'splitting', 'DR');

% AA
[x_draa, v_draa, res_norm_draa, ~, t6] = srls_GMC_acc(y, X,  lambda_ratio, 'type', 'group','groups',groups,...
                  'acceleration', "aa2","gamma",gamma, 'splitting', 'DR');

save("results/cvg_group.mat","t1","t2","t3","t4","t5","t6","res_norm_fb","res_norm_fbaa",...
