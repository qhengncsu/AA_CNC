...
for i = 1:length(lambda1_range)
    for j = 1:length(lambda2_range)
        ...
        for k = 1:n_folds
            ...
            [xhat_DY, res_norm_hist_DY] = cvx_min(A_train, Y_train(:), mu, z0, app, 'lambda1', lambda1_range(i), 'lambda2', lambda2_range(j),...
...
csvwrite('results/cv_scores_DY.csv',cv_scores)
[xhat_DY, res_norm_hist_DY] = cvx_min(A_meta_train, Y_meta_train(:), mu, z0, app, 'lambda1', best_lambda1, 'lambda2', best_lambda2,...
                                                 'max_iter', 1e4, 'splitting', 'DY', 'acceleration', 'original');
Xhat = reshape(xhat_DY, [p, L]);                                         
...
save("results/DY_CV.mat","best_lambda1","best_lambda2","max_r2","r2_test")