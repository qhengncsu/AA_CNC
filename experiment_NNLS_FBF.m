...
%%
t0 = tic;
[xhat_aa, res_norm_hist_aa] = cvxmin(A, b, mu, z0, app, 'max_iter', 1e4, 'splitting', 'FBF');
t_aa = toc(t0);
%%
t0 = tic;
[xhat, res_norm_hist] = cvxmin(A, b, mu, z0, app, 'max_iter', 1e4, 'acceleration', 'original', 'splitting', 'FBF');
t = toc(t0);
%% objective value
...
legend(['Original FBF (', num2str(round(t, 2)), ' s)'],...      
       ['AA + FBF (',num2str(round(t_aa, 2)), ' s)'], ...
       'FontSize', fsz,...
       'Location', 'best');
